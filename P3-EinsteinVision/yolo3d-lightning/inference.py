...
import time
sys.path.append("../")
from Networks.DataLoader import  get_frame
...
def inference(config: DictConfig):
    ...
    cap = cv2.VideoCapture(config.get('source_dir'))
    ...
    for idx in range(0, num_frames, 5):
        ...
        for det in dets:
            obj_cls = det['label'].split(" ")[0].capitalize()
            ...
            json_pack = {
                "type": str(obj_cls),
                "truncated": float(obj.truncation),
                "bbox": {'xmin': float(obj.xmin), 'ymin': float(obj.ymin), 'xmax': float(obj.xmax), 'ymax': float(obj.ymax)},
                "dimension": {'h': float(obj.h), 'w': float(obj.w), 'l': float(obj.l)},
                "location": {'x': float(obj.tx), 'y': float(obj.ty), 'z': float(obj.tz)},
                "rotation_global": float(obj.rot_global),
                "rotation_local": float(rot_local),

            }

            # write results
            if config.get("save_txt"):
                json_data.append({'frame': idx, "objects": json_pack})
        ...
        if config.get("save_result"):
            ...
            plot3dbev.save_plot(config.get("output_dir"), f'rendered_{idx}.png')

    if config.get("save_txt"):
        # save json results
        with open(f'{config.get("output_dir")}/results.json', "w") as f:
            json.dump(json_data, f, indent=4)

    # print time
    for key, value in avg_time.items():
        if key in ["detector", "regressor", "plotting"]:
            avg_time[key] = value / len(num_frames // 5)

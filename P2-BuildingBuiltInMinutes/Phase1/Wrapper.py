...
from LinearTriangulation import linearTriangulation2, linearTriangulation
...
if __name__ == '__main__':
    ...
    for i in range(C_out.shape[0]):
        ...
        ax.plot(C_out[i, 0], C_out[i, 1], marker='x', color='k', markersize=10)
    ...
    fig = plt.figure()
    ax = fig.add_subplot(111)
    ax.scatter(triangulated_points[:, 0], triangulated_points[:, 1], c='b', marker='o')
    ax.plot([0], [0], marker='x', color='k', markersize=10)
    ax.plot(C[0], C[1], marker='o', color='g', markersize=10)
    ax.set_xlabel('X')
    ax.set_ylabel('Y')
    plt.show()
